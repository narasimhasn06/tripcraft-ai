...
4. **Configure Google OAuth Provider (Required for "Continue with Google")**:
   * Go to the **[Google Cloud Console](https://console.cloud.google.com/)**, create a project, and set up the OAuth Consent Screen.
   * Under **Credentials**, create an **OAuth 2.0 Client ID** (Application Type: Web Application).
   * Copy the **Redirect URI** provided in the Supabase Google Provider dashboard (looks like `https://<project-id>.supabase.co/auth/v1/callback`) and paste it into Google's **Authorized redirect URIs**.
   * Go to your **Supabase Dashboard** -> **Authentication** -> **Providers** -> **Google**.
   * Toggle **Enable Google Provider** to **`ON`**.
   * Paste the **Client ID** and **Client Secret** generated by Google Cloud Console into the Supabase Google Provider settings, and click **Save**.
