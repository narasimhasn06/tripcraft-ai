...
export default function LandingPage() {
  ...
  return (
    <div className="flex-1 flex flex-col min-h-screen bg-slate-950 text-slate-100 overflow-hidden relative">
      ...
      {/* Navigation Header */}
      <header className="max-w-7xl w-full mx-auto px-6 py-5 flex items-center justify-between z-50 border-b border-slate-900/50 bg-slate-950/40 backdrop-blur-xl sticky top-0">
        <Link href="/" className="flex items-center gap-2.5 text-xl font-bold tracking-tight text-white group" aria-label="TripCraft AI Home">
      ...
      <main className="max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 flex-1 flex flex-col items-center justify-center text-center mt-12 sm:mt-20 z-10">
        ...
        {/* Sub-headline pill */}
        <div className="inline-flex items-center gap-2 px-3.5 py-1 bg-indigo-500/10 border border-indigo-500/25 text-indigo-300 rounded-full text-xs font-semibold mb-6 animate-pulse" style={{ animationDuration: '3s' }}>
          <Sparkles className="h-3.5 w-3.5 text-indigo-400" />
          <span>Custom Daily Itineraries in Seconds</span>
        </div>

        {/* Emotional travel-planning headline */}
        <h1 className="text-4xl sm:text-6xl md:text-7xl font-extrabold tracking-tight text-white max-w-4xl leading-[1.1] sm:leading-none">
          Escape the Planning. <br className="hidden sm:inline" />
          <span className="bg-gradient-to-r from-indigo-300 via-purple-300 to-emerald-300 bg-clip-text text-transparent">Embrace the Journey.</span>
        </h1>
        ...
        <div className="mt-10 flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
          <Link href={user ? "/dashboard" : "/auth"} className="w-full sm:w-auto">
            <Button size="lg" className="w-full sm:w-auto shadow-lg shadow-indigo-650/20 hover:-translate-y-0.5 transition-all" rightIcon={<ArrowRight className="h-4.5 w-4.5" />}>
              Plan My Trip
            </Button>
          </Link>
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full max-w-5xl mt-24 text-left px-4">
          <Card className="p-6 border-slate-800/80 bg-slate-900/20 hover:border-indigo-500/20 transition-all duration-300">
            <div className="p-3 bg-indigo-500/10 rounded-xl text-indigo-400 w-fit mb-4">
              <Sparkles className="h-5.5 w-5.5" />
            </div>
            <h3 className="text-lg font-bold text-white mb-2">AI-Powered Curation</h3>
            <p className="text-xs text-slate-450 leading-relaxed">
              Advanced algorithms analyze your budget, pace, and interests to build a perfectly balanced day-by-day itinerary.
            </p>
          </Card>
          <Card className="p-6 border-slate-800/80 bg-slate-900/20 hover:border-purple-500/20 transition-all duration-300">
            <div className="p-3 bg-purple-500/10 rounded-xl text-purple-400 w-fit mb-4">
              <Compass className="h-5.5 w-5.5" />
            </div>
            <h3 className="text-lg font-bold text-white mb-2">Interactive Customization</h3>
            <p className="text-xs text-slate-450 leading-relaxed">
              Every detail is editable. Tweak activities, adjust times, modify locations, and save changes to the cloud.
            </p>
          </Card>
          <Card className="p-6 border-slate-800/80 bg-slate-900/20 hover:border-emerald-500/20 transition-all duration-300">
            <div className="p-3 bg-emerald-500/10 rounded-xl text-emerald-400 w-fit mb-4">
              <Globe className="h-5.5 w-5.5" />
            </div>
            <h3 className="text-lg font-bold text-white mb-2">Pace & Budget Controls</h3>
            <p className="text-xs text-slate-450 leading-relaxed">
        ...
        <section id="preview" className="w-full max-w-4xl mt-24 sm:mt-32 text-left space-y-6 scroll-mt-24 mb-24 px-4">
          <div className="text-center max-w-lg mx-auto space-y-2 mb-10">
            <h2 className="text-3xl font-extrabold text-white tracking-tight">Curated Sample Plan</h2>
            <p className="text-sm text-slate-400 leading-relaxed">
              Here is a demonstration Kyoto itinerary tailored to a relaxed, culture-focused travel style.
            </p>
          </div>

          <Card className="border-slate-850/60 shadow-2xl relative overflow-hidden bg-slate-900/30">
            <div className="absolute inset-0 bg-gradient-to-tr from-indigo-500/3 via-transparent to-emerald-500/3 pointer-events-none" />
            
            {/* Kyoto Preview Title */}
            <CardHeader className="flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-slate-850/50 gap-4">
              <div>
            ...
            <CardContent className="mt-6 space-y-6">
              ...
              <div className="p-4 bg-slate-950/60 border border-slate-850/60 rounded-xl flex items-start gap-3">
                <Sparkles className="h-5 w-5 text-indigo-400 shrink-0 mt-0.5" />
                <div>
                  <span className="text-[10px] font-bold uppercase tracking-wider text-slate-450 block">
                    Day Focus Theme
                  </span>
                  <span className="text-sm font-semibold text-slate-200 leading-normal">
                ...
                {SAMPLE_ITINERARY.days[activeDay].activities.map((activity, actIdx) => {
                  ...
                  return (
                    <div key={actIdx} className="relative group">
                      ...
                      <div className="absolute -left-[32.5px] top-1.5 w-3.5 h-3.5 rounded-full border-4 border-slate-950 bg-indigo-500 group-hover:scale-125 transition-transform" />

                      <div className="bg-slate-950/20 hover:bg-slate-950/50 border border-slate-900 hover:border-slate-800 rounded-xl p-5 transition-all duration-300">
                        <div className="flex flex-col sm:flex-row justify-between sm:items-start gap-4">
                          ...
                          <div className="text-xs text-slate-500 flex items-center gap-1.5 shrink-0 self-start sm:self-auto bg-slate-950/40 px-2.5 py-1 rounded-lg border border-slate-880/40">
                            <MapPin className="h-3.5 w-3.5 text-indigo-500" />
                            <span>{activity.location}</span>
                          </div>
                        </div>

                        <div className="flex gap-4 mt-4 pt-3.5 border-t border-slate-900/60 text-[10px] text-slate-450 font-medium">
      ...
      {/* Footer */}
      <footer className="border-t border-slate-900 py-8 z-10 bg-slate-950/60">
        <div className="max-w-7xl w-full mx-auto px-6 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-slate-500">
          ...
          <div className="flex items-center gap-4">
            ...
            <a href="#" className="hover:text-slate-350 transition-colors flex items-center gap-1">
